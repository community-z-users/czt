
/*
ParserUtilsTest.java
JUnit based test
 *
Created on 15 February 2007, 10:11
 */
package net.sourceforge.czt.parser.circusconf;

//~--- non-JDK imports --------------------------------------------------------

import junit.framework.*;


//~--- classes ----------------------------------------------------------------

/**
 *
 * @author leo
 */
public class ParserUtilsTest extends TestCase {

    // private static SectionManager sm_ = new SectionManager(Dialect.CIRCUSCONF);
    public ParserUtilsTest(String testName) {
        super(testName);
    }

    //~--- methods ------------------------------------------------------------

    public void testParsingZ() {

        // String filename = "C:\\research\\tools\\java\\sourceforge\\czt\\0.5.2\\trunk\\parser\\target\\test-classes\\tests\\circusconf\\debug";
        // Term term = net.sourceforge.czt.parser.z.ParseUtils.parse(new FileSource(filename), sm_);
    }
}


//~ Formatted by Jindent --- http://www.jindent.com
