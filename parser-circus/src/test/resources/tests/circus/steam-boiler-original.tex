% sbwaiting must persist
% what about other parameters?

\documentclass{report}

\usepackage[color,lucida,zed]{csp}
\usepackage{my-a4}
...
\newcommand{\Circus}{\textsf{\slshape Circus}}
\newcommand{\HandelC}{\textsf{\slshape Handel-C}}
\newcommand{\fuzz}{\emph{f}{\small\textsc{uzz}}}
...
\title{%
  A \Circus\ Steam Boiler
  \\ %
  \medskip %
  \emph{using the unifying theory of Z and CSP}
  } %
  ...
  \begin{tabular}{@{}l@{}l}
    ...
    \begin{minipage}{.7\textwidth}
      \begin{zed}
        ...
        \\ %
        TimeState \defs [~ time: Time ~]
        \\ %
        \Delta Timer \defs [~ TimeState; TimeState' \mid time' \geq
        time ~]
        \endblock %
      \end{zed}
    \end{minipage}
...
\[
  \t1 %
  \block %
    \processname{TCycle} \defs  %
    \block %
      time := time + 1 \mod cycletime; %
      \\ %
      \If time = 0 \Then \event{startcycle} \then \processname{SKIP} %
      \Else \processname{SKIP}\,;
      \\ %
      \event{clocktick} \then \processname{TCycle}
    \endblock %
  \endblock %
\]
...
\processname{TCycle}\, \):
\[
  \t1 \processname{Timer} \defs time := cyclelimit; %
  \processname{TCycle}
  \also %
  \mathbf{end} ~~ \event{Timer}
\]
...
\begin{zed}
  SState ::= \freetypesokay \mid \freetypesfailed
\end{zed}
...
\begin{zed}
  QSensor \defs Unit[SState][qa_1/a_1, qa_2/a_2, qst/st]
\end{zed}
Initially, the water-level sensor's lower and upper-bounds are set to
the extreme values and it is judged to be working.
\begin{zed}
  InitQSensor \defs [~ QSensor' \mid qa_1' = 0 \land qa_2' = C \land
  qst' = \freetypesokay ~]
\end{zed}
The steam-rate volume sensor is similar.
\begin{zed}
  VSensor \defs Unit[SState][va_1/a_1, va_2/a_2, vst/st]
  \also %
  InitVSensor \defs [~ VSensor' \mid va_1' = 0 \land va_2' = 0 \land
  vst' = \freetypesokay ~]
\end{zed}
...
\begin{zed}
  PState ::= \freetypepopen \mid \freetypepwaiting \mid
  \freetypepclosed \mid \freetypepfailed
\end{zed}
As before, the system maintains lower and upper-bounds for each pump
and a record of its current state.
\begin{zed}
  Pump0 \defs Unit[PState][pa_1/a_1, pa_2/a_2, pst/st]
\end{zed}
...
is zero if one of the following holds true: the pump is closed; the
...
of the following holds true: the pump is open; the pump has failed; or
...
\begin{zed}
  PumpOpen \defs [~ Pump0 \mid pst = \freetypepopen \implies pa_1
  = P \land pa_2 = P ~]
\end{zed}
...
\begin{zed}
  PumpWaitingOrClosed \defs
  \\ %
  \t1 [~ Pump0 \mid pst = \freetypepwaiting \lor pst =
  \freetypepclosed \implies pa_1 = 0 \land pa_2 = 0 ~]
\end{zed}
...
\begin{zed}
  InitPump \defs [~ PumpWaitingOrClosed' \mid pst' = \freetypepclosed
  ~]
\end{zed}
...
\begin{zed}
  PCState ::= \freetypepcflow \mid \freetypepcnoflow \mid
  \freetypepcfailed
  \also %
...
\begin{zed}
  POpenPCFlowOrFailed \defs
  \\ %
  \t1 [~ PumpCtr0 \mid pst = \freetypepopen \implies pcst =
...
\begin{zed}
  PWaitingPCNoFlowOrFailed \defs
  \\ %
  \t1 [~ PumpCtr0 \mid pst = \freetypepwaiting \implies pcst =
...
must hold.
\begin{zed}
  PClosedPCNoFlowOrFailed \defs
  \\ %
  \t1 [~ PumpCtr0 \mid pst = \freetypepclosed \implies pcst =
  \freetypepcnoflow \lor pcst = \freetypepcfailed~]
\end{zed}
...
\begin{zed}
  PFailedPCFlow \defs
  \\ %
  \t1 [~ PumpCtr0 \mid pst = \freetypepfailed \land pcst
...
\begin{zed}
  ...
  \t1
  \block %
    POpenPCFlowOrFailed \land PWaitingPCNoFlowOrFailed \land {} %
    \\ %
    PClosedPCNoFlowOrFailed \land PFailedPCFlow \land PFailedPCFlow
    \land PFailedPCFailed
  \endblock %
\end{zed}
...
is no water flowing.
\begin{zed}
  InitPumpCtr \defs [~ PumpCtr' \mid InitPump \land pcst' =
  \freetypepcnoflow ~]
\end{zed}
...
\begin{schema}{InitPumpCtrSystem}
  PumpCtrSystem'
  \where %
  \exists InitPumpCtr \spot
  \\ %
  \t1 \forall i: PumpIndex \spot pumpctr'~i = \theta PumpCtr'
\end{schema}
...
\begin{zed}
  VState ::= \freetypevopen \mid \freetypevclosed
  \also %
  Valve \defs [~ valve: VState ~]
  \also %
  InitValve \defs [~ Valve' \mid valve' = \freetypevclosed ~]
\end{zed}
...
dynamics.
\begin{zed}
  CValues \defs [~ qc_1, qc_2, vc_1, vc_2: \nat ~]
\end{zed}
Initially, these calculations are as loose as possible.
\begin{zed}
  InitCValues \defs [~ CValues' \mid qc_1' = 0 \land qc_2' = C \land
  vc_1' = 0 \land vc_2' = W ~]
\end{zed}
...
level must be zero.
\begin{zed}
  QLowerBoundValveOpen \defs [~ CValues; Valve \mid valve = \freetypevopen
  \land qc_1 = 0 ~]
\end{zed}
...
\begin{zed}
  ...
  \t1
  \block %
    {}[~ CValues; QSensor; VSensor; Pump; Valve \mid valve = %
    \freetypevclosed \land {}
    \\ %
    \t1 qc_1 = max \{ 0, qa_1 - 5 * va_2 - 12 * U_1 + 5 * pa_1 \} ~]
  \endblock %
\end{zed}
...
\begin{zed}
  ...
  \t1
  \block %
    {}[~ CValues; QSensor; VSensor; Pump \mid %
    \\ %
    \t1 qc_2 = min \{ C, qa_2 - 5 * va_1 + 12 * U_2 + 5 * pa_2 \} ~]
  \endblock %
\end{zed}
...
\begin{zed}
  VLowerBound \defs [~ CValues; VSensor \mid vc_1 = max \{ 0, va_1 - 5
  * U_2 \} ~]
\end{zed}
The upper-bound for the steam-rate sensor is calculated in a similar
way to its lower-bound.
\begin{zed}
  VUpperBound \defs [~ CValues; VSensor \mid vc_2 = min \{ W, va_2 - 5
  * U_1 \} ~]
\end{zed}
...
\begin{zed}
  QFailed \defs [~ QSensor \mid qst = \freetypesfailed ~]
  \also %
  VFailed \defs [~ VSensor \mid vst = \freetypesfailed ~]
  \also %
  PFailed \defs
  \\ %
  \t1 [~ PumpCtrSystem \mid (~ \exists i: PumpIndex \spot (
...
free type.
\begin{zed}
  UnitFailure ::= \freetypeqfail \mid \freetypevfail \mid
  \freetypepfail \ldata PumpIndex \rdata \mid \freetypepcfail \ldata
  PumpIndex \rdata
  \also %
  Failures \defs [~ failures, noacks: \power UnitFailure \mid noacks
  \subseteq failures ~]
\end{zed}
...
\begin{schema}{EquipmentFailures}
  ...
  \block %
    ...
    \block %
      ...
      (~ u = \freetypepcfail~i \land PCFailed ~) \spot u ~\}
    \endblock %
  \endblock %
\end{schema}
...
reports.
\begin{zed}
  InitFailures \defs [~ Failures' \mid noacks' = \emptyset ~]
\end{zed}
...
\begin{zed}
  ...
  \\ %
  \t1 [~ Failures; failureacks: \power UnitFailure \mid failureacks
  \subseteq noacks ~]
  \also %
  AcceptFailureAcks \defs
  \\ %
  \t1 [~ \Delta Failures; FailuresExpected \mid noacks' = noacks
...
\begin{zed}
  ...
  \\ %
  \t1 [~ Failures; repairs: \power UnitFailure \mid repairs
  \subseteq failures ~]
  \also %
  AcceptRepairs \defs
  \\ %
...
\begin{zed}
  InputSignal ::=
  \\ %
  \t1 \freetypestop \mid \freetypesteamBoilerWaiting \mid
  \freetypephysicalUnitsReady \mid \freetypetransmissionFailure
\end{zed}
...
\begin{zed}
  Expected \defs [~ x?, a_1, a_2 : \nat \mid a_1 \leq x? \leq a_2 ~]
  \also %
...
consideration.
\begin{zed}
  Sensor \defs [~ \Delta Unit[SState]; c_1, c_2, c_1', c_2', x?: \nat
  ~]
\end{zed}
...
\begin{zed}
%  QExpected \defs QSensor \land Expected[q?/x?, qa_1/a_1, qa_2/a_2]
%  \\ %
%  QUnexpected \defs QSensor \land Unexpected[q?/x?, qa_1/a_1,
%  qa_2/a_2]
%  \also %
  CheckAndAdjustQ \defs
  \\ %
  \t1
  \block %
    QSensor \land {} %
  ...
  \endblock %
%  \also %
%  VExpected \defs VSensor \land Expected[v?/x?, va_1/a_1, va_2/a_2]
%  \\ %
%  VUnexpected \defs VSensor \land Unexpected[v?/x?, va_1/a_1,
%  va_2/a_2]
  \also %
  CheckAndAdjustV \defs
  \\ %
  \t1
  \block %
    VSensor \land {} %
...
\begin{zed}
  ...
  \block %
    ...
    \block %
      ...
      i: PumpIndex  \spot
      \\ %
      \t1 PromotePumpCheck \land CheckAndAdjustPump
    \endblock %
  \endblock %
\end{zed}
...
\end{table}
\[
  \chan\ \mathsl{levelbelowmin}, \mathsl{levelabovemax} %
  \also %
  \chan\
  \begin{block}
    \mathsl{emergencystop}, \mathsl{failures}, \mathsl{levelokay},
    \mathsl{nonqfailure}, \mathsl{physicalunitsready},
    \mathsl{qfailure},
    \\ %
    \mathsl{sbwaiting}, \mathsl{vzero}: \boolean
  \end{block}
\]
...
respectively.
\[
  \chan\ \mathsl{failuresrepairs}: ( \power UnitFailure ) \cross %
  ( \power UnitFailure )
\]
Finally, the abstract channel that represents the flow of information
about pump commands must also be replaced.
\[
  \chan\ {pumps}: PumpIndex \fun InputPState %
\]
\[
  Information = \chanevents{ %
    \begin{block}
      \mathsl{emergencystop}, \mathsl{failures},
      \mathsl{levelabovemax}, \mathsl{levelbelowmin},
      \mathsl{levelokay},
      \\ %
      \mathsl{nonqfailure}, \mathsl{physicalunitsready},
      \mathsl{qfailure}, \mathsl{sbwaiting}, \mathsl{vzero} }
  \end{block}
  \]

\[
\mathbf{process}\ \mathsl{Analyser} \defs %
\\ %
\cbegin
\]
...
\begin{flushleft}
  ...
  \begin{minipage}{.923\textwidth}
    \begin{zed}
      PumpOp \defs \Xi QSensor \land \Xi VSensor \land \Xi Valve \land
...
working limits.
\begin{zed}
  \t1 DangerZone \defs [~ AnalyserState \mid qa_1 \geq M_1 \land qa_2
  \leq M_2 \implies qa_1 < N_1 \land N_1 < qa_2 ~]
\end{zed}
\begin{flushleft}
  $\t1$
  \begin{minipage}{.923\textwidth}
...
N_2 \).
\begin{zed}
  \t1 LevelBelowMin \defs [~ AnalyserState \mid M_1 \leq qa_1 < N_1
  \land qa_2 \leq N_2 ~]
\end{zed}
...
\begin{zed}
  \t1 OtherPhysicalUnitsFail \defs \lnot QFailed \land \lnot
  AllPhysicalUnitsOkay
\end{zed}
\begin{zed}
  \t1 SteamBoilerWaiting \defs [~ AnalyserState \mid
  \freetypesteamBoilerWaiting \in signalhistory ~]
\end{zed}
\begin{zed}
  \t1 PhysicalUnitsReady \defs [~ AnalyserState \mid
  \freetypephysicalUnitsReady \in signalhistory ~]
\end{zed}
...
\[
  \t1 %
  \block %
    \mathsl{Analyser} \defs InitAnalyserState;  %
    \mathsl{AnalyserCycle}
  \endblock %
\]
\[
  \t1 %
  \block %
    \mathsl{AnalyserCycle} \defs %
    \\ %
    \t1
    \block %
      \mathsl{startcycle} \then
      \\%
      \t1
      \block %
        \event{input}\,?msg \then %
        ...
        \t1
        \block %
          SetPumpCtr \land PumpOp;
          \\ %
          Analyse; %
          \\ %
          \event{startexec} \then \mathsl{InfoService}
        \endblock %
      \endblock %
    \endblock %
  \endblock %
...
\[
  \t1
  \block %
    \mathsl{InfoService} \defs %
    \\ %
    \t1
    \block %
      \mathsl{OfferInformation}\,; \mathsl{InfoService}%
      \\ %
      \extchoice
      \\ %
      \event{failuresrepairs}\,!(noacks,repairs) \then
      \\ %
      \t1
      \block %
        \mathsl{pumps}\,?pumpstate \then pumpState := pumpstate; %
        \\ %
        \mathsl{AnalyserCycle}
      \endblock %
    \endblock %
...
\[
  \t1 %
  \block %
    \mathsl{OfferInformation} \defs %
    \\%
    \t1
    \block %
      \event{emergencystop}.EmergencyStop \then \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{sbwaiting}.SteamBoilerWaiting \then
      \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{vzero}.RateZero \then \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      LevelBelowMin \guard \event{levelbelowmin} \then
      \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      LevelAboveMax \guard \event{levelabovemax} \then
      \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{levelokay}.LevelInRange \then
      \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{physicalunitsready}.PhysicalUnitsReady
      \then \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{failures}.\lnot AllPhysicalUnitsOkay \then
      \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{qfailure}.QFailed \then \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{nonqfailure}.OtherPhysicalUnitsFail \then
      \mathsl{SKIP}
    \endblock %
  \endblock %
  \\ %
...
\[
  \t1 %
  \block %
    \mathbf{process} ~~ \mathsl{TAnalyser} \defs \also %
    \t1
    \block %
    ( \mathsl{Timer} \parallel[ TAnalyserInterface ] \mathsl{Analyser} %
    ) \hide TAnalyserInterface
    \endblock %
  \endblock %
\]
It is easy to see that this assembly is free from deadlock and
livelock, and this is confirmed by FDR.
\[
...
FDR code for the \( \mathsl{Analyser} \)\/ is contained in
appendix~\ref{appendix:FDR-code}.
...
\begin{zed}
  Mode ::= \freetypeinitialisation \mid \freetypenormal \mid
  \freetypedegraded \mid \freetyperescue \mid \freetypeemergencyStop
\end{zed}

\[
  \chan\ \mathsl{startpumps}, \mathsl{stoppumps}, \mathsl{openvalve}, %
  \mathsl{closevalve}, \mathsl{sendprogready}
  \also %
  Reports \defs \{ \mathsl{startpumps}, \mathsl{stoppumps},
  \mathsl{openvalve}, \mathsl{closevalve}, \mathsl{sendprogready} \}
  \also %
  TAControllerInterface \defs \mathsl{startexec} \cup Information
\]

\[
\mathbf{process} ~~ \mathsl{Controller} \defs %
\]
\vspace{-5.3ex}
\begin{zed}
  \t1 ModeState \defs [~ mode: Mode ~]
\end{zed}
...
\[
  \t1 %
  \block %
    \mathsl{EnterMode}\,(m: Mode) \defs \event{reportmode}\,!m \then %
    mode := m
  \endblock %
\]
...
\, \).
\[
  \t1 \mathsl{Controller} \defs mode := \freetypeinitialisation; %
  \mathsl{ControllerCycle}
\]
...
\[
  \t1 %
  \block %
    \mathsl{ControllerCycle} \defs %
    \\ %
    \t1
    \block %
      \event{startexec} \then \event{startreport} \then
      \mathsl{NewModeAnalysis}\,; %
      \\ %
      \mathsl{AdjustLevel}\,;
      \\ %
      \mathsl{endreport} \then \mathsl{ControllerCycle}
    \endblock %
    \also %
    \mathsl{NewModeAnalysis} \defs %
    \\ %
    \t1
    \block %
      \event{emergencystop.true} \then %
      \mathsl{EnterMode}\,(\freetypeemergencyStop)
      \\ %
      \extchoice
      \\ %
      \event{emergencystop.false} \then
      \\ %
      \t1
      \block %
        \If mode = \freetypeinitialisation \Then %
        \mathsl{InitModeAnalysis}
        \\ %
        \Else\ \If mode = \freetypenormal \Then
        \mathsl{NormalModeAnalysis}
        \\ %
        \Else\ \If mode = \freetypedegraded \Then
        \mathsl{DegradedModeAnalysis}
        \\ %
        \Else\ \If mode = \freetyperescue \Then
        \mathsl{RescueModeAnalysis}
        \\ %
        \Else \mathsl{SKIP}
      \endblock %
    \endblock %
...
\[
  \t1 %
  \begin{block}
    \mathsl{InitModeAnalysis} \defs
    \\
    \t1
    \begin{block}
      \event{sbwaiting.true} \then
      \\
      \t1
      \begin{block}
        (\
        \begin{block}
          \event{vzero.true} \then
          \\ %
          \t1
          \begin{block}
            (\
            \begin{block}
              \event{qfailure.false} \then
              \\ %
              \t1
              \begin{block}
                (\
                \begin{block}
                  \event{physicalunitsready.true} \then
                  \\ %
                  \t1
                  \begin{block}
                    (\
                    \begin{block}
                      \event{levelokay.true} \then
                      \\ %
                      \t1
                      \begin{block}
                        (\
                        \begin{block}
                          \event{failures.false} \then
                          \mathsl{EnterMode}\,(\freetypenormal)
                          \\ %
                          \extchoice
                          \\ %
                          \event{failures.true} \then
                          \mathsl{EnterMode}\,(\freetypedegraded)
                        ~)
                        \end{block}
                      \end{block}
                      \\ %
                  ...
                  \extchoice
                  \\ %
                  \event{physicalunitsready.false} \then
                  \\ %
                  \t1
                  \begin{block}
                    (\
                    \begin{block}
                      \event{levelokay.true} \then
                      \event{sendprogready} \then \mathsl{SKIP}
                      \\ %
                      \extchoice
                      \\ %
                      \event{levelokay.false} \then \mathsl{SKIP}\
                    ~)
                ~)
                    \end{block}
                  \end{block}
      ...
      \extchoice
      \\ %
      \event{sbwaiting.false} \then \mathsl{SKIP}
    \end{block}
  \end{block}
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{NormalModeAnalysis} \defs %
    \\ %
    \t1
    \begin{block} %
      \event{failures.false} \then \mathsl{SKIP} %
      \\ %
      \extchoice
      \\ %
      \event{qfailure.true} \then
      \processname{EnterMode}\,(\freetyperescue)
      \\ %
      \extchoice
      \\ %
      \event{nonqfailure.true} \then
      \processname{EnterMode}\,(\freetypedegraded)
    \end{block} %
  \end{block} %
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{DegradedModeAnalysis} \defs %
    \\ %
    \t1
    \begin{block} %
      \event{qfailure.false} \then
      ...
      \begin{block} %
        (\
        \begin{block}
          \event{failures.true} \then \mathsl{SKIP}
          \\ %
          \extchoice
          \\ %
          \event{failures.false} \then
      ...
      \extchoice
      \\ %
      \event{qfailure.true} \then \mathsl{EnterMode}\,(\freetyperescue) %
    \end{block} %
  \end{block} %
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{RescueModeAnalysis} \defs %
    \\ %
    \t1
    \begin{block} %
      \event{qfailure.true} \then \mathsl{SKIP}
      \\ %
      \extchoice
      \\ %
      \event{qfailure.false} \then %
...
\[
  \t1 %
  \begin{block} %
    \mathsl{AdjustLevel} \defs %
    \\ %
    \t1
    \begin{block} %
      \event{levelbelowmin} \then \mathsl{RaiseLevel} %
      \\ %
      \extchoice
      \\ %
      \event{levelabovemax} \then \mathsl{ReduceLevel}
      \\ %
      \extchoice
      \\ %
      \event{levelokay.true} \then \mathsl{RetainLevel}
    \end{block} %
  \end{block} %
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{RaiseLevel} \defs %
    \\ %
    \t1
    \begin{block} %
      \mathsl{StartPumps}\,; %
      \\ %
      \If mode = \freetypeinitialisation \Then \mathsl{CloseValve}
      \Else \mathsl{SKIP}
    \end{block} %
  \end{block} %
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{ReduceLevel} \defs %
    \\ %
    \t1
    \begin{block} %
      \mathsl{StopPumps}\,; %
      \\ %
      \If mode = \freetypeinitialisation \Then \mathsl{OpenValve}
      \Else \mathsl{SKIP}
    \end{block} %
  \end{block} %
\]
...
\[
  \t1 %
  \begin{block} %
    \mathsl{RetainLevel} \defs %
    \\ %
    \t1
    \begin{block} %
      \mathsl{StopPumps}\,; %
      \\ %
      \If mode = \freetypeinitialisation \Then \mathsl{CloseValve} \Else
      \mathsl{SKIP}
    \end{block} %
  \end{block} %
\]
...
\[
  \t1
  \begin{block} %
    \mathsl{StartPumps} \defs \event{startpumps} \then \mathsl{SKIP} %
    \\ %
    \mathsl{StopPumps} \defs \event{stoppumps} \then \mathsl{SKIP} %
    \\ %
    \mathsl{OpenValve} \defs \event{openvalve} \then \mathsl{SKIP} %
    \\ %
    \mathsl{CloseValve} \defs \event{closevalve} \then \mathsl{SKIP} %
  \end{block} %
\]
This completes the description of the \( \mathsl{Controller} \, \).
\[
  \mathbf{end} ~~ \mathsl{Controller} %
\]

\[
  \mathbf{process} ~~ \mathsl{TAController} \defs \also %
  \t1
  \begin{block} %
    ( \mathsl{TAnalyser}
    \parallel[ TAControllerInterface ]
    \mathsl{Controller} ) \hide TAControllerInterface
  \end{block} %
\]
...
\item Signals drawn from the following free type.
  \begin{syntax}
    OutputSignal & ::= & \freetypeprogramReady
    \\ %
    & \mid &  \freetypeopenValve
    \\ %
    & \mid &  \freetypecloseValve
    \\ %
    & \mid &  \freetypelevelFailureDetection
    \\ %
    & \mid & \freetypesteamFailureDetection
    \\ %
    & \mid & \freetypelevelRepairedAcknowledgement
    \\ %
    & \mid & \freetypesteamRepairedAcknowledgement
  \end{syntax}
...
\[
  \chan\ \mathsl{output}: OutputMsg %
\]
The process is defined as follows.
\[
  \mathbf{process}\ \mathsl{Reporter} \defs %
\]
The process waits for the start of the report phase; after this, it
provides its report service.
\[
  \t1 %
  \mathsl{Reporter} \defs \event{startreport} \then %
  \mathsl{ReportService} %
\]
...
\[
  \t1
  \begin{block} %
    \mathsl{ReportService} \defs %
    \\ %
    \t1
    \begin{block} %
      \mathsl{GatherReports}\,; \mathsl{ReportService} %
      \\ %
      \extchoice %
      \\ %
      \mathsl{reportmode}.\mathsf{emergencyStop} \then mode :=
      \mathsf{emergencyStop}; \mathsl{TidyUp}
      \\ %
      \extchoice %
      \\ %
      \mathsl{TidyUp}
...
\[
  ...
  \begin{block}
    ...
    \t1
    \begin{block}
      \mathsl{failuresrepairs}\,?(noacks,repairs) \then FailuresRepairs;
      \\ %
      \begin{block}
        \mathsl{output}\,!( \theta OutputMsg ) \then
        \\ %
        \t1
        \begin{block}
          pumps\,!pumpState \then
...
\[
  \t1 %
  \begin{block} %
    \mathsl{GatherReports} \defs %
    \\ %
    \t1
    \begin{block} %
      \Extchoice m: Nonemergency \spot\event{reportmode}.m \then mode := m
      \\ %
      \extchoice
      \\ %
      \event{sendprogready} \then signals := signals \cup \{
      \freetypeprogramReady \}
      \\ %
      \extchoice
      \\ %
      \event{startpumps} \then pumpState := PumpIndex \cross \{
      \freetypepopen \}
      \\ %
      \extchoice
      \\ %
      \event{stoppumps} \then pumpState := PumpIndex \cross \{
      \freetypepclosed \}
      \\ %
      \extchoice
      \\ %
      \event{openvalve} \then signals := signals \cup \{
      \freetypeopenValve \}
      \\ %
      \extchoice
      \\ %
      \event{closevalve} \then signals := signals \cup \{
      \freetypecloseValve \}
    \end{block} %
  \end{block} %
  \also %
\mathbf{end}\ \mathsl{Reporter} %
\]
The reporter is added to the other components.  The result is free
from deadlock and livelock.
\[
  \mathsl{TACReporter} \defs %
  \\ %
  \t1 ( \mathsl{TAController} \parallel [ TACReporterInterface ]
  \mathsl{Reporter} ) \hide TACReporterInterface
  \also %
...
\[
  \mathsl{SteamBoiler} \defs \mathsl{TACReporter}
\]
...
\[
  ...
  \begin{block}
    ...
    \begin{block}
      ...
      \t1
      \begin{block}
        \mathsl{emergencystop}.false \then
        \\ %
        \t1
        \begin{block}
          \mathsl{sbwaiting}.false \then
          \\ %
          \t1
          \begin{block}
            \mathsl{levelokay}.true \then
            \\ %
            \t1
            \begin{block}
              \mathsl{stoppumps} \then
              \\ %
              \t1
              \begin{block}
                \mathsl{closevalve} \then
                \\ %
                \t1
                \begin{block}
                  \mathsl{endreport} \then
  ...
  \begin{block}
    ...
    \begin{block}
      \mathsl{Controller}
      \\ %
      \parallel[ \{ \mathsl{levelabovemax,} \mathsl{levelbelowmin,}
      \mathsl{sbwaiting}.true, \mathsl{emergencystop}.true \} ]
      \\ %
      \mathsl{STOP}
    \end{block}
    \\ %
