 ...
 */
public class ZMLComparator implements Comparator
{
  ...
  private static boolean isNullOrEqual(String o1, String o2, String testname) {
	...
	*/
	...
	int nl1AnnsCount = 0;
	int nl2AnnsCount = 0;
	int shorterNodelistLength = n1.getLength() < n2.getLength() ? n1.getLength() : n2.getLength();

	for (int i = 0; i < shorterNodelistLength; i++) {
	    Node curNode = n1.item(i+nl1AnnsCount);
	    if (curNode.getNodeName().equals("Anns")) {
		nl1AnnsCount++;
		break;
	    }
	    ...
	    Node compNode = n2.item(i+nl2AnnsCount);
	    if (compNode.getNodeName().equals("Anns")) {
		nl2AnnsCount++;
		break;
	    }
