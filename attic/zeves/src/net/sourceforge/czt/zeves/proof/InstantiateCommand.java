...
public class InstantiateCommand  extends AbstractProofCommand {
    ...
    InstantiateCommand(CZT2ZEvesPrinter zprinter, ZRefName variable, Expr value) {
        super(zprinter, variable, value);
    }
    
    InstantiateCommand(CZT2ZEvesPrinter zprinter, List<ZRefName> variable, List<Expr> value) {
        super(zprinter, combine(variable, value));
    }
    
    private static Object[] combine(List<ZRefName> variables, List<Expr> values) {
    ...
    protected String format(Object... attrs) {
        ...
        while (i < attrs.length-1) {
            result.append(", ");
            result.append(fZPrinter.print((ZRefName)attrs[i]));
            result.append(" == ");
