  ...
  in Unicode markup using the {{{http://www.jedit.org} jEdit editor}}.

  For more information about CZT, please have a look at
  {{{http://czt.sourceforge.net/} our web site}}
  We encourage you to file bugs, ask for help, provide patches, etc.
  All this can be done starting from 
  {{{http://sourceforge.net/projects/czt/} our sourceforge site}}.
...
* Installing the jEdit plugins
  ...
  If you are working with a source distribution, follow the
  {{{../install.html} installation instructions}} to compile and install CZT.
  This creates jar files in <<<CZT_HOME/lib>>>, including the plugin jar files
  mentioned below.
  ...
  [[3]] copy <<<CZT_HOME/lib/ZLivePlugin.jar>>> to
        <<<JEDIT_SETTINGS_DIR/jars>>>
