...
public class AlloyPrinter extends VisitReturn<String>
{
	...
	public String visitSig (Sig sig) {
		...
		if (sig.isLone()) ret += "lone ";
		if (sig.isOne()) ret += "one ";
		if (sig.isSome()) ret += "some ";
		if (sig.isAbstract()) ret += "abstract ";

		ret += "sig " + sig.label();
		if (sig instanceof SubSig) ret += " extends " + visit(((SubSig) sig).parent());
	...
	public String visit(ExprConstant x)
	{
		...
		if (x.op() == ExprConstant.Op.FALSE) return visitThis(ExprConstant.ONE.equal(ExprConstant.ZERO));
		return x.toString();
	}
