...
//	 */
			...
			if (d instanceof VarDecl) {
				...
				for (Name name : nameList) {
					sig.addField(new Field(Z2Alloy.getInstance().print(name),
							Z2Alloy.getInstance().visit(vardecl.getExpr())));
				}
			} else if (d instanceof InclDecl) {
				InclDecl incldecl = (InclDecl) d;		
				Expr newPred = addInclSig((Sig) Z2Alloy.getInstance().visit(incldecl.getExpr()), sig);
				if (newPred != null && fieldPred != null) fieldPred = newPred.and(fieldPred);
				else if (newPred != null) fieldPred = newPred;
			} else {
	...
	/*
	 * note that the expr returned is the new predicate
	 * the fields and stuff are added within this method
	 */
	private Expr addInclSig(Sig inclSig, Sig sig) {
		...
		// the fields needed for calling the predicate of the included sig at the end
		...
		for (Field subfield : inclSig.fields()) {
			if (!sigfieldnames.containsKey(subfield.label())) { // ie they aren't already present
			...
			// put the field in args - doesn't matter if it comes from the old or new location
		...
		// this case is if it is a primed - the correct pred to find is the unprimed version
