...
public class Module implements Iterable<Object>{
	...
	public Sig getSig (String label) {
		if (sigByLabel.containsKey(label)) return sigByLabel.get(label);
		for (Module m : includedModules) {
			if (m.sigByLabel.containsKey(label)) return m.sigByLabel.get(label);
		}
		return null;
	}

	public Fact getFact (String label) {
		if (factByLabel.containsKey(label)) return factByLabel.get(label);
		for (Module m : includedModules) {
			if (m.factByLabel.containsKey(label)) return m.factByLabel.get(label);
		}
		return null;
	}

	public Func getFunc (String label) {
		if (funcByLabel.containsKey(label)) return funcByLabel.get(label);
		for (Module m : includedModules) {
			if (m.funcByLabel.containsKey(label)) return m.funcByLabel.get(label);
		}
		return null;
	}

	public Command getCommand (String label) {
		if (commandByLabel.containsKey(label)) return commandByLabel.get(label);
		for (Module m : includedModules) {
			if (m.commandByLabel.containsKey(label)) return m.commandByLabel.get(label);
		}
		return null;
	}
