 ...
 */
public class PostChecker extends Checker<net.sourceforge.czt.typecheck.z.ErrorAnn>
...
//      ActionType actionType = (ActionType)typeRefName;
  ...
  private net.sourceforge.czt.typecheck.z.ErrorAnn checkCallProcessParamTypes(ZDeclName procName, List<NameTypePair> decs, List<Type2> types) {
    ...
    if(decs.size() == types.size()) {
      for(NameTypePair pair : decs) {
        ...
        if(foundU instanceof UnknownType) {
          Object [] params = {assertZDeclName(currentProcess()).getWord(), procName.getWord(), i+1};
          errorAnn = errorAnn(procName, ErrorMessage.PARAM_PROC_CALL_UNDECLARED_VAR, params);
          //result = false;
          break;
        }
        if (unify(foundU, expectedU) != SUCC) {
          Object [] params = {assertZDeclName(currentProcess()).getWord(), expectedU, foundU, i, procName.getWord()};
      ...
      Object [] params = {assertZDeclName(currentProcess()).getWord(), decs.size(), types.size(), procName.getWord()};
  ...
  private net.sourceforge.czt.typecheck.z.ErrorAnn checkCallProcess(CallProcess term, List<NameTypePair> paramsOrIndexes) {    
    ...
    // CallType.Param, CallType.Index
        ...
        if (term.getCallType().equals(CallType.Param)) {
            if(!kindOfProcess.equals("PARAM")){
              ...
              errorAnn = checkCallProcessParamTypes(procDecl, paramsOrIndexes, typeExprs);
            }
        } else {
  ...
  private net.sourceforge.czt.typecheck.z.ErrorAnn checkCallAction(CallAction term, List<NameTypePair> params) {
