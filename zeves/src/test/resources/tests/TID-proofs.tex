...
use finiteFunction[Y, X][f := (\_ \inv) [X, Y] f];
use functionFinite[Y, X][A := B, B := A, f := (\_ \inv) [X, Y] f];
prove;
\end{zproof}
...
\begin{theorem}{rule lOptionalSubtyping}[X]
\forall  A: \power  X @ \forall  x: \Optional A @ x \in  \Optional X
\end{theorem}

\begin{zproof}[lOptionalSubtyping]
invoke (\Optional \_);
rewrite;
...
rewrite;
use lOptionalSubtyping[(\lblot baseCertId: (\lblot issuer: USER \rblot);cid:
...
\begin{zproof}[unnamed\$n4\$domainCheck]
prove;
\end{zproof}
...
\begin{zproof}[unnamed\$n5\$domainCheck]
prove by reduce;
\end{zproof}
...
\begin{zed}
ValidateUserTokenOK \defs BioCheckNotRequired \lor  BioCheckRequired
\end{zed}
...
apply lInTheDomain to predicate authCert\_\_0 \in \dom \The;
use
  lOptionalSubtyping[\lblot baseCertId: \lblot issuer: USER \rblot;
...
\begin{theorem}{lTISPollPREStdUpdLemma}
   ...
   TISPoll[adminToken := adminToken',
                        ...
                        minPreservedLogSize:=minPreservedLogSize', %duplicate: now:=now',
