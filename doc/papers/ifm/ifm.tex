...
\section{Animation}\label{animation}
    ...
    CZT also includes animation facilities with the ZLive tool.
    Z animation is particularly useful for testing, rapid prototyping,
    and experimenting.

    Other main advantages for animating Z specifications are finite
    theorem proving (or theorem testing), and model checking.
    Yet restricted to finite state models, animation allows validation,
    verification, and experimentation with Z specifications.
    An extensive discussion and comparison among Z animation tools
    available are given in~\cite{utting-jaza}.

\subsection{Extending ZLive}

    ZLive is an animator capable of evaluating predicates and
    expressions using mode and subtype analysis~\cite{winikooff98},
    where its architecture is based on a previous Z animator implemented in
    ...
    These moded-predicates are then reordered according to the cheapest solution
    order in terms of number of solutions. Finally, all solutions are lazily
    ...
    While performing mode analysis, the main issue is to find the best
    mode with respect to the complexity of solutions it represents.
    Currently ZLive only implements three modes:~all-free
    ...
    must be defined regardless the order.

    We envisage the following possibilities in using ZLive for a new Z extension:
    \begin{description}
        \item[Explicit Inclusion:] Each new Z construct, as well as further
            evaluation algorithms, are \textbf{directly} added to ZLive \textbf{via interface
            and class inheritance} similarly to the CZT typechecker.

        \item[Transformation to Standard Z:] If each new Z construct can be transformed
            back into standard Z \textbf{using rewriting rules}, then ZLive can be used directly.
            This is the approach taken by the proposed Object-Z animator~\textbf{*(any reference?)*}.
            This approach is of rewriting specifications is similar to the Z
            \textbf{refinement calculus}~\cite{z.others:ana.phd,fm.ref:morgan}.

        \item[Operational Semantics:] If the operational semantics
            of the new language can be given in standard Z, one can use ZLive
            directly to animate the new Z extension. Although it is \textbf{a
            meta-level approach for execution}, its performance should not
            suffer much as the semantics is animated using standard Z in
            ZLive directly. This approach is taken for animating the
            operational semantics of \Circus~\cite{circus.mc:opsem}.
    \end{description}
    Depending on the new language constructs to be animated, these possibilities
    can be combined. Furthermore, we expect to include in ZLive predicate transformers,
    schema unfolding, and a tactic language in the spirit of ANGEL~\cite{z.others:angel}.
    These improvements enable a basis for a theorem prover for standard Z
    and its extensions that is open-source and cross-platform.
...
\subsection{Extension Example: Animating \Circus}
    ...
    In this architecture ZLive is used from two different perspectives:~(i)~to
    the Z part of \Circus\ specifications, and~(ii)~to evaluate the operational
    semantics of \Circus\ given in Z while performing the model checking search.
    The former extends ZLive available flattened predicates in order to cover
    most frequently Z constructs used in \Circus\ specifications, whereas the
    latter directly uses available constructs to animate the \Circus\ semantics.

    For example, as both \Circus\ specifications and the language operational
    semantics are heavily based on the schema calculus, extension of ZLive with
    with schema unfolding and operators such as $\theta$ is very important.
    This illustrates the first and third approaches for extending ZLive
    we have mentioned earlier. Although the approach on transformation rules
    can avoid necessary extensions, it demands the rewriting laws to be proved.
    As \Circus\ is heavily based on the notion of stepwise refinement,
    the second approach fits nicely with the philosophy of \Circus.
    ...
    The theorem proving module in the model checker presents both in the
    compiler and refinement engine dispatches requests for evaluation of Z
    expressions and predicates. These are either verification conditions over
    the state operations defined in Z, or possible enabling paths available
    for investigation from the behavioural actions given using CSP.
    They are both given as standard Z statements from the operational semantics of \Circus.
    At this point theorem proving is usually necessary to discharge proof obligations,
    and transform expressions or predicates. Nevertheless, for specifications
    with simple state operations, animation is also a good idea that can improve
    the automation levels of the model checking process.
    ...
    With few improvements and extensions to the current implementation of
    the schema calculus, it should be possible to automatically model check
    simple-state \Circus\ specifications with in ZLive.
    ...
    Nevertheless, the fact ZLive architecture itself already envisage a term
    ...
    evaluated elsewhere.
    This is a good \textbf{example of how to integrate different CZT tools across
    different paradigms and tool boundaries}, from standard Z parsing through
    extended typechecking and animation.
...
\begin{itemize}
    \item[Petra:] The very first ideas for conclusions; probably needs rewriting.
\end{itemize}

  In this paper, we presented reuse and extensibility mechanisms that
  makes the CZT framework an ideal starting point to develop new
  integrated formal methods tools for Z and its extensions.  It has
  shown how the XML schemas for Z and its extensions support reuse of
  all or parts of the language constructs provided.  The generation
  and design of the AST classes is presented.  Using examples from
  Object-Z, TCOZ, and \Circus, it has been shown how the CZT tools like
  parser, typechecker, and animators developed for Standard Z can be
  reused in a way that minimises code duplication and maintenance.
  The strategies presented can also help developers of integrated
  formal methods tools not based on Z to make their framework as
  extensible as possible.

  The process of including a new extension within the CZT framework for
  parsing and typechecking is divided as follows:
  \begin{description}
    \item[1. Extended XML Schema:] Provide a XML schema containing the new language constructs.
        Depending on the extension, this can import the base standard Z like \Circus\ does, or
        an already available extension like TCOZ does with Object-Z. Provided the extension
        language has a well structure BNF syntax, the \textbf{XML schema} should not
        \textbf{take} more than \textbf{a week to be built}. A good design is one that reuse
        as much as possible the available XML schema constructs.

    \item[2. New Parsing and Scanning Rules:] Add to the XML template files the new parsing and
        scanning rules. At this point extensions to the standard toolkit and keywords are also
        included. Provided one is familiar with Java CUP and JFlex, including new rules should
        not be very demanding as most of the precedence and reduction problems comes from
        standard Z complex productions such as operator templates.

    \item[3. Extended Checkers:] Inherit (or create) necessary checker classes for new constructs.
        As the \textbf{flexible typechecking architecture allows considerable code reuse}, the overall
        task should be straightforward.
  \end{description}

  Z parsing and typechecking is neither a novel idea, nor a unavailable resource.
  Nevertheless, flexible and integrated open-source support for ISO standard Z
  heavily focused on strong conformance and extensibility was not yet available.
  The \textbf{philosophy CZT advocates} is simple: \textbf{provide an open-source
  framework with a set of tools for editing, parsing, typechecking and animating
  formal specifications written in Z with support for Z extensions}.
  As new extensions are included and the framework matures, we expect it to become
  the common base for a growing number of strongly conformed tools for Z and its extensions.
