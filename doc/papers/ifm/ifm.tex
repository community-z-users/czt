...
\label{parsers}
  ...
  \Circus\ and TCOZ parsers, as well as a Object-Z typechecker, are
...
\subsection{Parsers and Scanners}
  ...
  Unicode or \LaTeX\ markup.  Support for e-mail markup is planned. To
  produce different parsers and scanners for Z and its extensions, CZT
  uses XML templates, and generates the code from these templates
  using XSLT\footnote{See~\url{http://www.w3.org/TR/xslt}}, a language
  for transforming XML. Java
...
%
For example, an initial definition loading environment for the \Circus\
operational semantics given in~\cite{circus.mc:opsem} has been developed following the
guidelines for Z and Object-Z typecheckers.
The formal type-rules for \Circus\ are under development~\cite{circus.other:typechecker}.
Provided the architecture and the design decisions are understood, the
implementation task of a \Circus\ typechecker should be considerably simplified,
and directly benefit from code reuse and elegant object-oriented design.
The obvious advantage is that by inheriting from the base Z typechecker,
a possible \Circus\ typechecker implementation already enforces standard Z
type-checking conformance. Therefore, one can concentrate on the implementation
of new type-rules for \Circus.

In conclusion, CZT supports extendability in its typecheckers by:
\begin{description}
 \item[Using Visitors]: Visitors are used to apply type rules ---
  providing a straightforward way to implement type rules for new
  constructs, or overriding existing type rules.
 \item[Sharing Common Code:] Methods used throughout the typechecker
 are shared in an abstract super class that is inherited by all
 visitors.
...
\section{Conclusions and Future Work} \label{sec:conclusions}
  ...
  Other tools as well as extensions for CZT are planned.
  ...
  in our agenda. These improvements would enable a basis for a theorem
