...
with PASS and FAIL being incomparable.  All provisos must satisfy three
properties:
\begin{enumerate}
\item the value of the proviso must depend only upon the text of the
  proviso and (for the lookup proviso) the contents of the current
  Z section and its ancestor sections.
...
\item if the text of the proviso is ground (does not contain jokers),
...
preserve this validity property.  Next we discuss several such tools
that are provided by CZT.
...
states: \emph{PASS} (the proviso was correctly discharged),
\emph{FAIL} (the proviso is false), \emph{UNKNOWN} (the
...
this section to rewrite $E$ to $E2$, which is usually a schema
construction, $[D | P]$.
