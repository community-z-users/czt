...
The ZedRules notation provides four kinds of side conditions.
...
\item[Typecheck:] $\proviso E:BT$, where $E$ is an expression and $BT$
  is a carrier set that corresponds to a Z base type.  Note that $BT$
...
This tool is a Java class that provides a simple depth-first search
method that can be used to find proofs automatically.
...
applications of rules from the given set of rewrite rules.
...
object logic of $\V$.  To supporting richer logics such as these,
