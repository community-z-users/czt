...
with jokers.
%Their informal semantics is that the conclusion is true
...
%it reserves the name forever, making that name unusable as an ordinary Z name.

%A rule can refer to several kinds of symbols:
%those of the core Z language,
%such as $\power$, $\nat$ and $\varg = \varg$;
%those defined explicitly in Z,
%such as $dom$, $\varg \cat \varg$ and $foo$ (however those are defined),
%and jokers declared by a joker declaration paragraph in a section.
%
...
%
%References to jokers are typechecked using the environment
%where the rule is written.
%This environment is extended as local variables declared inside a rule
%are typechecked.
%Other references are typechecked using the environment
...
%Note that a reference to a name such as $dom$,
%which has a definition in the standard toolkit,
%could be found by the dynamic phase to refer to
%a different definition of $dom$,
%even if the standard toolkit is in scope
%in the section where the rule is written.
...
reasoning.
%Provisos are written using the syntax of Standard Z predicates
%(though with different semantics) extended with jokers.
%These are Standard Z predicates extended with jokers, but
...
%distinguish them from antecedent predicates.
...
%  This calculates the type of the
...
%  given $Name$ does not appear in $Term$.
...
%  is a normal predicate rather than a proviso.
...
%All provisos must satisfy the following property.
...
%  Given the models for the environment where the rule is applied.  A rule $R$ is valid iff for every ground
...
%write is valid according to this definition.
Proof tools are
