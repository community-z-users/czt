...
public class Gaffe extends JFrame
{
  ...
  protected Gaffe(final String[] args)
  {
    ...
    getContentPane().add(new JLabel("Please choose animate or design mode:",
                                    JLabel.CENTER));
    JButton animate, design, quit;

    animate = new JButton("Animate");
    getContentPane().add(animate);     animate.setMnemonic(KeyEvent.VK_A);

    design = new JButton("Design");
    getContentPane().add(design);      design.setMnemonic(KeyEvent.VK_D);

    quit = new JButton("Quit");
    getContentPane().add(quit);        quit.setMnemonic(KeyEvent.VK_Q);
  ...
  {
    //chop off args[0]
    return (String[]) Arrays.asList(args).subList(1, args.length)
