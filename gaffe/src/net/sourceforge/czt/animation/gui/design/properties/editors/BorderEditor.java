...
public class BorderEditor extends PropertyEditorSupport
{
  ...
  public BorderEditor()
  {
    ...
    try {
      ...
      chooserDialog.getContentPane().add(setupCenterPane(),
                                         BorderLayout.CENTER);
      chooserDialog.getContentPane().add(setupExamplePane(),
                                         BorderLayout.EAST);
  ...
  private SingleBorderEditor matteBorderEditor = new SingleBorderEditor() {
      ...
      {
        setEditingBorder(BorderFactory.createMatteBorder(1, 1, 1, 1,
                                                         Color.black));
        setLayout(new GridBagLayout());
        add(new JLabel("Top:"));    add(topS);
        add(new JLabel("Left:"));   add(leftS);
  ...
  private JPanel setupCenterPane()
  {
    ...
    //Setting up the button pane
    {
      ...
      addButton.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent ev)
          {
            final SingleBorderEditor editor
              = (SingleBorderEditor) mainPane.getSelectedComponent();
            Border newBorder;
            if (editor == null) return;
            newBorder = editor.createBorder();
            if (titleCheck.isSelected())
              newBorder
                = BorderFactory.createTitledBorder(newBorder,
                                                   titleField.getText());
            ...
            if (exampleBorder.getBorder() == null)
              ...
              Border b
                = BorderFactory.createCompoundBorder(newBorder,
