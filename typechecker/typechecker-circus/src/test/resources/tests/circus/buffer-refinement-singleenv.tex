...
For $StoreInput$ the schema expression brackets are mandatory
because it relies on the input variable $x?$. All others just rely on
the global state, which is always within scope, so could afford not 
to have the surrounding brackets, hence the (somewhat harmless) 
warnings.
%
\begin{circus}
