...
public class Transformer implements
	TermVisitor, TermAVisitor, SpecVisitor, ZSectVisitor,
	FreeParaVisitor, FreetypeVisitor,
	ExistsPredVisitor, IffPredVisitor, ImpliesPredVisitor, OrPredVisitor, ExprPredVisitor,
	LetExprVisitor, IffExprVisitor, ImpliesExprVisitor, OrExprVisitor, CondExprVisitor,
...
*/
	 ...
	 * put elements in front of injections
	 */
	...
	public Object visitCondExpr(CondExpr term) {
		...
		members.add(left);
		members.add(right);
		SetExpr set_ext = factory_.createSetExpr(members);

		DeclName dn = factory_.createDeclName();
		dn.setWord("_i");
		Vector vars = new Vector();
		vars.add(dn);
		VarDecl var = factory_.createVarDecl(vars, set_ext);
	...
	/**
	 * transforms a const decl into a var decl
	 */
