...
*. whenever a type equation throws an exception, should add a variable type to its type anns
*. name of any ZSect cannot be any of the prelude and toolkit sections!
*. change the search methods in TypeEnv and SectTypeEnv to search for DeclName instead of String (done!)
*. integrate type environment with the type checker: PARTIALLY DONE
*. change the structure, maybe make typechecker a superclass of typechecker for z, oz and tcoz...
	so that common elements such as spec and read write file can be implemented in just 1 place
*/
...
public class TypeChecker
  implements TermVisitor, TermAVisitor, SpecVisitor, ZSectVisitor, ParentVisitor,
  	GivenParaVisitor, DeclNameVisitor,
  	AxParaVisitor, ConstDeclVisitor, VarDeclVisitor, //InclDeclVisitor,
  	SchTextVisitor,
  	SetExprVisitor, SetCompExprVisitor, PowerExprVisitor, TupleExprVisitor,
  	TupleSelExprVisitor, BindExprVisitor, ThetaExprVisitor, BindSelExprVisitor,
  	ApplExprVisitor, MuExprVisitor,
  	SchExprVisitor, NegExprVisitor
  	//SectTypeEnvAnnVisitor //AndPredVisitor, ForallPredVisitor //ZVisitor
{
	public static String repository = "../toolkit";
	...
	private SectTypeEnv sectTypeEnv;
	...
	public TypeChecker () {
		...
		transformer = new Transformer(factory_);
	}

	public Term checkFile(String fileName) throws Exception {
	...
	private Term checkFile(File file) {
	    Term spec = (Term) reader.read(file);
	...
	public Object visitZSect(ZSect term) {
		...
		if (term instanceof TermA) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (sectAnn instanceof SectTypeEnvAnn) {
					sectTypeEnv.checkAndAdd((SectTypeEnvAnn) sectAnn);
				}
			}
		}
		...
		Vector pare_names = new Vector();
		for (int i = 0; i < parents.size(); i++) {
			Parent pare = (Parent) parents.get(i);
			String curName = pare.getWord();
			if (! (sect_name.equals(curName) || pare_names.contains(curName))) {
				pare_names.add(curName);
	...
	// assumption: names in givenpara cannot contain stroke
  	public Object visitGivenPara(GivenPara term) {
		...
		GivenTypeEq gte = new GivenTypeEq(typeEnv, term, this);
		try {
		...
		for (int i = 0; i < declNames.size(); i++) {
			...
			try {
				typeEnv.addNameTypePair(ntPair);
			} catch (TypeException e) {
				parent_ = tmpPare;
				System.out.println(e.toString());
			}
			...
			nstTriple = factory_.createNameSectTypeTriple(temp, sect_name, curPType);
			try {
				sectTypeEnv.addNameSectTypePair(nstTriple);
			} catch (TypeException e) {
				parent_ = tmpPare;
				System.out.println(e.toString());
			}
	...
	public Object visitDeclName(DeclName term) {
		// DeclName in GivenPara (GivenType) or AxPara (GenType) cannot contain strokes
	...
	public Object visitAxPara(AxPara term) {
		...
		AxParaTypeEq apte = new AxParaTypeEq(typeEnv, term, this);
		parent_ = term;
		try {
			apte.solve();
		} catch (TypeException e) {
	...
	public Object visitSetExpr(SetExpr term) {
		SetExprTypeEq setq = new SetExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitSetCompExpr(SetCompExpr term) {
		SetCompExprTypeEq scetq = new SetCompExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitSchText(SchText term) {
		SchTextTypeEq sttq = new SchTextTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitConstDecl(ConstDecl term) {
		...
		VarDecl vdecl = (VarDecl) ((VarDecl) transformer.visitConstDecl(term)).accept(this);
	...
	public Object visitVarDecl(VarDecl term) {
		VarDeclTypeEq vdte = new VarDeclTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitPowerExpr(PowerExpr term) {
		PowerExprTypeEq petq = new PowerExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitTupleExpr(TupleExpr term) {
		TupleExprTypeEq tetq = new TupleExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitTupleSelExpr(TupleSelExpr term) {
		TupleSelExprTypeEq tsetq = new TupleSelExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitBindExpr(BindExpr term) {
		BindExprTypeEq betq = new BindExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitThetaExpr(ThetaExpr term) {
		ThetaExprTypeEq tetq = new ThetaExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitBindSelExpr(BindSelExpr term) {
		BindSelExprTypeEq bsetq = new BindSelExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitApplExpr(ApplExpr term) {
		ApplExprTypeEq aetq = new ApplExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitMuExpr(MuExpr term) {
		MuExprTypeEq metq = new MuExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitSchExpr(SchExpr term) {
		SchExprTypeEq setq = new SchExprTypeEq(typeEnv, term, this);
		try {
	...
	public Object visitNegExpr(NegExpr term) {
		NegExprTypeEq netq = new NegExprTypeEq(typeEnv, term, this);
		try {
...
//---------------------------------------------------------------------------//
	...
	private File findFile(Parent term) throws TypeException {
		...
		try {
			result = new File(repository, name);
		} catch (NullPointerException e) {
			try {
				result = new File(curDir, name);
	...
	public static boolean unify(Type type1, Type type2) {
		...
		if (type1 instanceof GenType || type1 instanceof GivenType) {
			...
			for (int i = 0; i < t11.size(); i++) {
				...
				if (unify(t12, t22) == false) return false;
			}
			return true;
		}
	...
	// s1 and s2 are two Lists of Strokes
	...
	// add annotation (mainly type ann) to a TermA
