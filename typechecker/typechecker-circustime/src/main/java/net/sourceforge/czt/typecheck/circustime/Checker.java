  ...
  extends net.sourceforge.czt.typecheck.circus.Checker<R>
{
  ...
  protected CircusCommunicationList typeCheckPrefixFollowingAction(Action1 term, List<NameTypePair> inputVars)
  {
	  ...
	  if (term instanceof PrefixingTimeAction)
	  {
		  ...
		  // enter scope for variable name @N. If not in place, it will be an empty scope
		  typeEnv().enterScope();
		  ...
		  else
		  {
			  ...
			  error(pta, ErrorMessage.CIRCUS_TIME_UNKNOWN_PREFIXTIMEACTION, 
					  new Object[] { getCurrentProcessName(), getCurrentActionName(), pta.getClass().getSimpleName() });
		  }
