...
import net.sourceforge.czt.circustime.util.PrintVisitor;
import net.sourceforge.czt.parser.util.ErrorType;
import net.sourceforge.czt.session.Markup;
import net.sourceforge.czt.session.Dialect;
import net.sourceforge.czt.session.SectionManager;
import net.sourceforge.czt.typecheck.circustime.ErrorAnn;
import net.sourceforge.czt.util.CztLogger;

/**
 * A JUnit test class for testing the typechecker. This reads any files not
 * ending with a "_" from the directory tests/circus.
 * 
 * If the file ends with ".error", then the test reads everything up to the
 * first "-" and that is the name of the error constant.
 * 
 * If the file does not end in ".error" or "_", then no error is expected.
 * 
 * @author Leo Freitas
 */
public class TypeCheckerTest extends
		net.sourceforge.czt.typecheck.circus.TypeCheckerTest {
	...
	protected static Level DEBUG_LEVEL = DEBUG_TESTING ? Level.FINEST
			: VERBOSE ? Level.WARNING : Level.SEVERE;
	protected static List<String> TESTS_SOURCEDIR = new ArrayList<String>();

	static {
		File shouldDebug = new File(
				"src/test/resources/tests/circustime/debug-please");
		try {
			if (VERBOSE) {
				System.out.println("shouldDebug? \n path = "
						+ shouldDebug.getPath() + "\n abs path = "
						+ shouldDebug.getAbsolutePath() + "\n can path = "
						+ shouldDebug.getCanonicalPath() + " \n exists? = "
						+ shouldDebug.exists());
			}
	...
	private final SectionManager manager_ = new SectionManager(
			Dialect.CIRCUSTIME);
	...
	protected void collectTests(TestSuite suite, String directoryName) {
		...
		for (String path : paths) {
			String fullDirectoryName = path.trim()
					+ (!path.isEmpty() ? "\\" : "") + directoryName;
			if (VERBOSE) {
				System.out
						.println("Full directory name = " + fullDirectoryName);
			}
			...
			if (!directory.isDirectory()) {
				...
				if (url != null) {
					try {
						String urlPath = URLDecoder.decode(url.getFile(),
								"UTF-8");
						if (VERBOSE) {
							System.out.println("Looking for tests under: "
									+ urlPath + fullDirectoryName);
						}
					...
					fail("Could not retrieve a valid testing set under "
							+ directory.getAbsolutePath());
				}
			} else {
				files = directory.listFiles();
	...
	protected Term parse(String file, SectionManager manager) throws Exception {
		Term term = super.parse(new File(file).toURI().toURL(), manager);
		if (VERBOSE) {
			System.out.println("\tabout to parse as " + Markup.getMarkup(file)
					+ " file " + file);
		}
		if (DEBUG_TESTING && DEBUG_LEVEL.intValue() <= Level.INFO.intValue()) {
			...
			System.out
					.println("DEBUG: AFTER PARSING, PrintVisitor for " + file);
	...
	protected List<? extends net.sourceforge.czt.typecheck.z.ErrorAnn> typecheck(
			Term term, SectionManager manager) throws Exception {
				...
				for (net.sourceforge.czt.typecheck.z.ErrorAnn errorAnn : errors) {
					...
					if (errorAnn.getErrorType().equals(ErrorType.ERROR)) {
						fail("\nUnexpected type error" + "\n\tFile: " + file_
								+ "\n\tException: "
								+ errorAnn.getErrorMessage().toString()
				...
				for (net.sourceforge.czt.typecheck.z.ErrorAnn errorAnn : errors) {
					...
					if (errorAnn.getErrorType().equals(ErrorType.ERROR)) {
						actual = removeUnderscore(errorAnn.getErrorMessage()
								.toString());
						foundCorrectError = (exception_
								.compareToIgnoreCase(actual) == 0);
