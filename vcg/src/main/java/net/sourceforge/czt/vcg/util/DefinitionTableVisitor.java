             ...
             ZSectVisitor<DefinitionTable>
{
    ...
    throws CommandException
  {
    ...
    if (errors_.isEmpty())
      ...
      throw new DefinitionTable.DefinitionException(
  ...
  public DefinitionTable visitTerm(Term term)
  {
    ...
    throw new CztException(new DefinitionTable.DefinitionException(message,
  ...
  public DefinitionTable visitGivenPara(GivenPara para)
  {
    ...
    for(Name given : para.getZNameList())
    {
      ...
      if (given instanceof ZName)
      {
        ...
        addDefinition(factory_.createZNameList(),
                      givenName,
  ...
  public DefinitionTable visitBranch(Branch term)
  {
    ...
    addDefinition(factory_.createZNameList(), ftElem, ftExpr, DefinitionKind.FREETYPE);
    return null;
  }
  ...
  public DefinitionTable visitZSect(ZSect zSect)
  {
    ...
    for (Parent parent : zSect.getParent()) 
    {
      ...
      catch(CztException ex)
      {
        ...
        if (cause instanceof DefinitionTable.DefinitionException)
        {
          raiseUnsupportedCase("parent raised errors", null, parent);
          errors_.addAll(((DefinitionTable.DefinitionException)cause).getExceptions());
  ...
  protected void addDefinition(ZNameList genFormals, Name declName,
		  Expr defExpr, DefinitionKind defType)
  {
    ...
    DefinitionTable.Definition def =
      new DefinitionTable.Definition(sectName_, declName, genFormals, defExpr, unifType, defType);
    try {
      table_.add(declName, def);
      }
  ...
  private Name buildName(Name name, List<Stroke> strokes)
  {
    ...
    if (strokes != null && !strokes.isEmpty())
    {
      ...
      result = factory_.createZName(zname.getWord(),
        factory_.createZStrokeList(zname.getZStrokeList()));
      ((ZName)result).getZStrokeList().addAll(strokes);
    }
    return result;
  }

  List<DefinitionTable.DefinitionException> errors_ = new ArrayList<DefinitionTable.DefinitionException>();

  private void raiseUnsupportedCase(String message, DefinitionKind defKind, Term term)
  {
    LocAnn loc = term.getAnn(LocAnn.class);
    errors_.add(new DefinitionTable.DefinitionException(
    ...
    switch (exprKind)
    {
      ...
      case MIXED:
        ...
        else if (expr instanceof RefExpr)
        {
          ...
          DefinitionTable.Definition def = table_.lookup(rExpr.getName());
          if (def != null)
  ...
  {
    // for schema bindinds,
//    boolean localDecl = defKind.isSchemaBinding();
//    if (localDecl)
//    {
//      assert currentName_ != null;
//      DefinitionTable.Definition def = table_.lookup(currentName_);
//
//      // if schema already declared, this is a repeated occurrance - nothing to do
//      if (def != null)
//        return;
//    }

    // otherwise, either a global occurrance to add or the first time
    // for localc schema bindings. add each of them accordingly
          ...
          List<Stroke> strokes, DefinitionKind defKind)
  {
    ...
    Name bname = buildName(decl.getName(), strokes);
    addDefinition(genFormals, bname, defExpr, defKind);
  }
          ...
          Stack<Stroke> strokes, Stack<DefinitionKind> defKinds)
  {
    ...
    // versions of DeclList from the base name (e.g., without Delta/Xi)
    ZName refName;
    ...
    // process the reference: either check consistency, or add stroked Delta/Xi
    ...
    Name includedName = isSpecialRefName ? origName : buildName(origName, strokes);
    if (table_.lookup(includedName) == null)
    {
      // this would be the case names included but not defined: e.g., State' or \Delta State
      addDefinition(genFormals, includedName, refExpr, DefinitionKind.SCHEMAINCLUSION);// getSchInclusion(calee));
    }
          ...
          Stack<Stroke> strokes, Stack<DefinitionKind> defKinds)
  {
    ...
    DefinitionTable.Definition def = table_.lookup(refName);

    // if we find the name as a Schema declaration
    if (def != null)
    {
      Expr defExpr = def.getExpr();
      if (def.getDefinitionKind().equals(DefinitionKind.SCHEMADECL))
      {
        ...
        if (defExpr instanceof SchExpr)
        {
          ...
          if (!strokes.empty())
          {
            ...
            Name strokedName = buildName(refName, strokes);
            DefinitionTable.Definition strokedDef = table_.lookup(strokedName);

            // if already in the table, no need to process it
            // otherwise, add the stroked version of refName (pass refName)!
            if (strokedDef == null)
            {
              processSchExpr(genFormals, refName, schExpr, defKinds, strokes);
              addDefinition(genFormals, strokedName, schExpr, DefinitionKind.SCHEMAINCLUSION);
            }
          }
          ...
          Stack<Stroke> strokes, Stack<DefinitionKind> defKinds)
  {
    ...
    else if (inclDeclExpr instanceof RenameExpr)
    {
      ...
      // NEED TO DO SOMTHING HERE about renamings
      throw new RuntimeException("Not handling rename expr yet!");

      //processInclExpr(rexpr.getExpr(), genFormals, strokes, defKinds);
      ...
      switch (currentDefKind.value())
      {
        ...
        case DefinitionKind.SCHEMADECL_VALUE:
          ...
          if (decl instanceof ConstDecl)
          {
            ...
            addDefinition(genFormals, cDecl.getZName(), cDecl.getExpr(), currentDefKind);
          }
          else
          {
            // non-std schema
