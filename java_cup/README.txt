...
There is also a change to the emit class, which breaks up each case in
the switch statement (each case in the parse table) into its own
method, so the do_action method does not grow too large, thus removing
the "code too large" problem that javac raises when compiling the TCOZ
parser.
