  ...
  private static final String VIEW_ID = CZTPlugin.getPluginID() + ".views.ZInfo";
	...
	private Control createZViewer(Composite parent) {
		...
		SimpleZSourceViewerConfiguration configuration = new SimpleZSourceViewerConfiguration(
				CZTPlugin.getDefault().getCZTTextTools().getColorManager(), 
				store, null, IZPartitions.Z_PARTITIONING, false);
	...
	protected void setText(String text, Markup markup) {
		...
		CZTPlugin.getDefault().getCZTTextTools()
				.setupCZTDocumentPartitioner(document, IZPartitions.Z_PARTITIONING, fileType);
