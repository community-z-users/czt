...
//		super(JavaEditorMessages.JavaEditor_markOccurrences_job_name);
	...
	public IStatus run(IProgressMonitor progressMonitor) {
		...
		computeOccurrenceAnnotations(annotationMap, fEditor.getParsedData().getSpec(), fSelectedTerm, message);
		...
		synchronized (fEditor.getAnnotationLock(annotationModel)) {
			if (annotationModel instanceof IAnnotationModelExtension) {
				((IAnnotationModelExtension)annotationModel).replaceAnnotations(fEditor.getOccurrenceAnnotations(), annotationMap);
			} else {
			...
			fEditor.setOccurrenceAnnotations((Annotation[])annotationMap.keySet().toArray(new Annotation[annotationMap.keySet().size()]));
		}
