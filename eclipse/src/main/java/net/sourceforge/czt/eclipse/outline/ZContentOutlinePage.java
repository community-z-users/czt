 ...
 * @author Chengdong Xu
 */
public class ZContentOutlinePage extends ContentOutlinePage implements
				...
				if (document != null) {
					try {
						...
						if (newInput instanceof CztSegment) {
							for (CztSegment child : ((CztSegment) newInput)
									.getChildren()) {
								document.addPosition(SEGMENTS, child
										.getRange());
	...
	public void select(int offset) {
		...
		OutlineContentProvider contentProvider =
			(OutlineContentProvider)this.getTreeViewer().getContentProvider();
	...
	public Object getNodeOfPoint(OutlineContentProvider contentProvider, Object[] elements, int offset) {
		Object node = null;
		for (Object element : elements) {
			if (element instanceof CztSegment) {
				...
				if ((pos.getOffset() <= offset)
					...
					node = element;
					Object childNode = getNodeOfPoint(
							contentProvider,
							contentProvider.getChildren(element),
							offset);
					if (childNode != null)
						node = childNode;
					break;
