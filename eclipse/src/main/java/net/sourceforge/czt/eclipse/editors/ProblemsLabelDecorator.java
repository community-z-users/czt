...
public class ProblemsLabelDecorator implements ILabelDecorator, ILightweightLabelDecorator {
	...
	private ImageDescriptorRegistry getRegistry() {
		if (fRegistry == null) {
			fRegistry= fUseNewRegistry ? new ImageDescriptorRegistry() : CZTPlugin.getImageDescriptorRegistry();
		}
		return fRegistry;
	}
	...
	public Image decorateImage(Image image, Object obj) {
		...
		if (adornmentFlags != 0) {
			...
			return getRegistry().get(new CztElementImageDescriptor(baseImage, adornmentFlags, new Point(bounds.width, bounds.height)));
		}
		return image;
	}
		...
		try {
			if (obj instanceof CztSegment) {
				...
				if (model != null) {
					...
					result= getErrorTicksFromMarkers(((FileEditorInput)((ZEditor)source).getEditorInput()).getFile(), IResource.DEPTH_ONE, (CztSegment)obj);
				}
...
//			JavaPlugin.log(e);
	...
	private int getErrorTicksFromMarkers(IResource res, int depth, CztSegment sourceElement) throws CoreException {
	...
	private boolean isMarkerInRange(IMarker marker, CztSegment sourceElement) throws CoreException {
		if (marker != null && marker.exists() && marker.isSubtypeOf(IZMarker.PROBLEM)) {
	...
	private int getErrorTicksFromAnnotationModel(IAnnotationModel model, CztSegment sourceElement) throws CoreException {
		int info= 0;
		...
		while ((info != ERRORTICK_ERROR) && iter.hasNext()) {
			...
			if (annot instanceof MarkerAnnotation) {
				...
				if (isMarkerInRange(marker, sourceElement) && marker.isSubtypeOf(IZMarker.PROBLEM)) {
