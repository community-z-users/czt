 ...
 * 
 */
public class ZDoubleClickStrategy implements ITextDoubleClickStrategy {
	...
	/**
	 * Create a ZDoubleClickStrategy
	 */
	public ZDoubleClickStrategy() {
		super();
	}
...
//		if (!selectBracketBlock())
	...
	protected boolean matchWord() {
		...
		try {
			...
			if (ZCharacter.isZWhitespace(c))
				return false;
			
			while (pos >= 0) {
				c = document.getChar(pos);
				if (!ZCharacter.isZWordPart(c))
					break;
				--pos;
			}
