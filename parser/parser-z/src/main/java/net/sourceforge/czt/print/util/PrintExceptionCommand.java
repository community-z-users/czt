...
public class PrintExceptionCommand
  implements Command
{
  @Override
  ...
  {
    final Key<PrintException> key = new Key<PrintException>(name, PrintException.class);
    if (! manager.isCached(key)) {
      // parser exception has no dependencies
      manager.endTransaction(key, 
    		  new PrintException(manager.getDialect(), 
