...
public class FlatLessThan extends FlatPred
{
  ...
  public FlatLessThan(ArrayList newargs)
  {
    if (newargs == null || newargs.size() != 2)
      throw new IllegalArgumentException("FlatLessThan requires 2 args");
    args = newargs;
    solutionsReturned = -1;
  }
  ...
  public boolean inferBounds(Bounds bnds)
  {
    ...
    if (rmax != null)
      changed |= bnds.addUpper(left, rmax.subtract(BigInteger.ONE));

    // propagate lower bound from left to right.
    BigInteger lmin = bnds.getLower(left);
    if (lmin != null)
      changed |= bnds.addLower(right, lmin.add(BigInteger.ONE));
